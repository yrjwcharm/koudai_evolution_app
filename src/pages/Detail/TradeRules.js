 ...
 * @LastEditTime: 2021-01-19 16:48:40
...
class TradeRules extends Component {
  ...
  render_limit () {
    ...
    return (
      <View>
        {
          data.part5.length > 0 && data.part5.map((item, index) => {
            return (
  ...
  renderContent () {
    ...
    return (
      <View>
        {
          Object.keys(data).length > 0 && this.state.curIndex == 0 ? <View>
            ...
            <View style={[ styles.feeTableItem, Style.flexBetween, { backgroundColor: '#fff', borderBottomWidth: Space.borderWidth } ]}>
            ...
            <View style={[ styles.feeDescBox ]}>
              <View style={{ marginBottom: text(12) }}>
                <Html style={styles.feeDesc} html={'基金卖出时一般按照先进先出规则, 部分基金卖出按照后进后出规则. 基金卖出手续费与持有期限相关. 实际费用收取请以基金公司确认为准.'} />
              </View>
              <View style={{ marginBottom: text(20) }}>
                <Html style={styles.feeDesc} html={`赎回计算公式：\n赎回总额=赎回数量xT日基金单位净值\n赎回费用=赎回总额x赎回费率\n赎回到账金额=赎回总额-赎回费用`} />
              </View>
              <Html style={{ ...styles.feeDesc, lineHeight: text(20) }} html={'基金费率等信息以基金公司最新披露的基金信息为准'} />
            </View>
            <Text style={[ styles.title ]}>调仓费率</Text>
            <View style={[ styles.feeDescBox, { paddingTop: 0 } ]}>
              <Text style={[ styles.feeDesc, { color: Colors.descColor } ]}>内容内容捏内容内容，基金卖出时一般按照先进先出规则, 部分基金卖出按照后进后出规则. 基金卖出手续费与持有期限相关. 实际费用收取请以基金公司确认为准.</Text>
            </View>
          </View> : null
        }
        {
          Object.keys(data).length > 0 && this.state.curIndex == 1 ? <View>
            <View style={styles.productInfoWrap}>
              ...
              <View style={styles.buyTableWrap}>
                <View style={styles.buyTableHead}>
                  <Text style={[ styles.buyTableItem, styles.fontColor, { flex: 1.5 } ]}>{data.part2.buy.table.head.name}</Text>
                  <Text style={[ styles.buyTableItem, styles.fontColor ]}>{data.part2.buy.table.head.confirm}</Text>
                  <Text style={[ styles.buyTableItem, styles.fontColor ]}>{data.part2.buy.table.head.profile_time}</Text>
                </View>
                {
                  data.part2.buy.table.body.length > 0 && data.part2.buy.table.body.map((item, index) => {
                    return (
                      <View style={{ flexDirection: 'row', paddingVertical: text(10), backgroundColor: (index + 1) % 2 == 0 ? Colors.bgColor : '#fff' }} key={index + 'v'}>
                        <Text style={[styles.buyTableItem, { flex: 1.5 }]}>{item.name}</Text>
            ...
            <View style={styles.productInfoWrap}>
              ...
              <View style={[ styles.buyTableWrap]}>
                <View style={styles.buyTableHead}>
                  <Text style={[styles.buyTableItem, styles.fontColor, { flex: 1.5 }]}>{data.part2.redeem.table.head.name}</Text>
                  <Text style={[styles.buyTableItem, styles.fontColor]}>{data.part2.redeem.table.head.confirm}</Text>
                  <Text style={[styles.buyTableItem, styles.fontColor]}>{data.part2.redeem.table.head.profile_time}</Text>
                </View>
                {
                  data.part2.redeem.table.body.length > 0 && data.part2.redeem.table.body.map((item, index) => {
                    return (
                      <View key={index + 'g'} style={{ flexDirection: 'row', paddingVertical: 10, backgroundColor: (index + 1) % 2 == 0 ? Colors.bgColor : '#fff' }}>
                        <Text style={[styles.buyTableItem, { flex: 1.5 }]}>{item.name}</Text>
                        <Text style={styles.buyTableItem}>{item.confirm}</Text>
                        <Text style={styles.buyTableItem}>{item.profile_time}</Text>
                      </View>
        ...
        {
          Object.keys(data).length > 0 && this.state.curIndex == 2 ? <View>
            <View style={styles.productInfoWrap}>
              <Text style={styles.productInfoTitle}>{data.part3.buy.title}</Text>
              {
                data.part3.buy.desc.length > 0 && data.part3.buy.desc.map((_k, index) => {
                  return (
                    <View style={styles.buyNotice} key={index + '111i'}>
                      <View style={styles.blueCircle}></View>
                      <Html style={{ marginLeft: text(10), color: Colors.darkGrayColor }} html={_k}></Html>
                    </View>
                  );
                })
              }
              <View style={styles.buyTableWrap}>
                <View style={styles.buyTableHead}>
                  <Text style={[styles.buyTableItem, styles.fontColor]}>{data.part3.buy.table.head.name}</Text>
                  <Text style={[styles.buyTableItem, styles.fontColor]}>{data.part3.buy.table.head.start_amount}</Text>
                </View>
                {
                  data.part3.buy.table.body.length > 0 && data.part3.buy.table.body.map((item, index) => {
                    return (
                      <View style={{ flexDirection: 'row', paddingVertical: text(10), backgroundColor: (index + 1) % 2 == 0 ? Colors.bgColor : '#fff' }} key={index + 'baaa'}>
            ...
            <View style={styles.productInfoWrap}>
              ...
              {
                data.part3.redeem.desc.length > 0 && data.part3.redeem.desc.map((_i, index) => {
                  return (
                    <View style={styles.buyNotice} key={index}>
                      <View style={styles.blueCircle}></View>
                      <Text style={{ marginLeft: text(10), color: Colors.darkGrayColor }}>{_i}</Text>
                    </View>
                  );
                })
              }
              <View style={styles.buyTableWrap}>
                <View style={styles.buyTableHead}>
                  <Text style={[styles.buyTableItem, styles.fontColor]}>{data.part3.redeem.table.head.name}</Text>
                  <Text style={[styles.buyTableItem, styles.fontColor]}>{data.part3.redeem.table.head.min_redeem}</Text>
                  <Text style={[styles.buyTableItem, styles.fontColor]}>{data.part3.redeem.table.head.min_hold}</Text>
                </View>
                {
                  data.part3.redeem.table.body.length > 0 && data.part3.redeem.table.body.map((item, index) => {
                    return (
                      <View style={styles.buyTableBody} key={index + 'c'}>
                        <Text style={styles.buyTableItem}>{item.name}</Text>
        ...
        {
          Object.keys(data).length > 0 && this.state.curIndex == 3 ? <View style={{ backgroundColor: '#fff', paddingBottom: text(20) }}>
            {
              data.part4.length > 0 && data.part4.map((item, index) => {
                return (
                  <View style={styles.banklistWrap} key={index + 'l'}>
                    ...
                    <View style={styles.banklistItem}>
                      ...
                      <Text style={{ textAlign: 'right', flex: 1, color: Colors.descColor }}>{item.single_limit}/{item.daily_limit}/{item.month_limit}</Text>
                    </View>
                  </View>
                );
              })
  ...
  render () {
    ...
    return (
      <>
        {
          Object.keys(data).length > 0 && <ScrollableTabView
          ...
          >
            {
              data.head.length > 0 && data.head.map((item, index) => {
                return (
