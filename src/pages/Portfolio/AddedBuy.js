...
const AddedBuy = ({navigation, route}) => {
  ...
  const getNewConfig = useCallback(debounce(
    (amount) => {
  ...
  const handlerMoneyChange = useCallback((val) => {
    val = onlyNumber(val + '')
    setAmount(val);
  ...
  return Object.keys(data).length > 0 ? (
    <View style={{...styles.container}}>
        <ScrollView 
              style={{color: Colors.bgColor, paddingBottom: FixedBtn.btnHeight+ px(28)}}      
            >
            <Text style={styles.title}>理财计划</Text>
            {/* 追加金额 */}
            <View style={styles.addedMoney}>
              <Text style={styles.buyInfoTitle}>
                  {data.buy_info.title}
              </Text>
            ...
            <View style={styles.matching}>
                ...
                <View style={styles.matchRateChart}>
                    {
                      data.fund_list_src.map((item,idx)=>(
                        <View style={{backgroundColor: item.color, width: item.ratio.toFixed(4) * 100 + '%', height: px(24)}} key={idx}/>
                      ))
                    }
                </View>
              </View>
              {/* 箭头 */}
              {
                ...
                <View style={styles.downImg}>
                 <FastImage source={require('../../assets/img/down.png')} style={{width: px(28), height: px(28)}}/>
                </View>
                : null
              }
                {/* 追加后配比图 */}
